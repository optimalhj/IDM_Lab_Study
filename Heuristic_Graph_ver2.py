...
def SPT(names):
    ...
    spt_result = tardiness(spt)[1]
    return spt_result, "SPT"

# -----------------------------EDD-----------------------------------------

def EDD(names):
    edd = sorted(names, key=lambda name: name.due)
    edd_result = tardiness(edd)[1]
    return edd_result ,"EDD"
...
def SLACK(names):
    ...
    slack_result = tardiness(slack)[1]
    return slack_result, "SLACK"
...
if __name__ == "__main__":
    ...
    methods = [SPT, EDD, SLACK, IGA, SA_ANS_Swap, SA_ANS_Insert]
    for func in methods:
