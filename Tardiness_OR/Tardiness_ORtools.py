...
def tardiness(seqs):
    ...
    # start time
    st = {jb : md.NumVar(0, md.infinity(), name='st') for jb in seqs}

    # end time
    et = {jb : md.NumVar(0, md.infinity(), name='et') for jb in seqs}

    # tardiness
    td = {jb : md.NumVar(0, md.infinity(), name='td') for jb in seqs}
    ...
    for job_sq in range(len(seqs) - 1):
        md.Add(et[seqs[job_sq]] <= st[seqs[job_sq + 1]])

    # Tardiness == 0 or Tardiness == (End - Due)
    for jb in seqs:
        md.Add(et[jb] - jb.due <= td[jb])

    '''
    z = md.NumVar(0, md.infinity(), name='z')
    final_obj = md.Constraint(0,md.infinity())
    for job in seqs:
        final_obj.SetCoefficient(td[job], -1)
    final_obj.SetCoefficient(z,1)
    md.Minimize(z)
    '''
    md.Minimize(sum(td.values()))
    md.Solve()

    return seqs, md.Objective().Value()
...
def build(names,times,dates):
    new_set = [Jobs(names[i],times[i],dates[i]) for i in range(len(names))]
    return new_set
