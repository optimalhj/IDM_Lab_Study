...
def solve_cp_sat(jobs):
    ...
    for job in jobs:
        ...
        interval = md.NewIntervalVar(start,job.processing_time,end, "Interval_%s"%job.name)
        td = md.NewIntVar(0, horizon, "tardiness_%s"%job.name)
...
if __name__ == "__main__":
    ...
    if status in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
        ...
        for job in ini_set:
            assigned_jobs.append({"job" : job ,"start" : starts[job], "end" : ends[job], "duration" : job.processing_time, })
        assigned_jobs.sort(key=lambda x: x["start"])

        for job in assigned_jobs:
            print("start :", job["start"], "/ end :", "/ duration:", job["duration"], job["end"], "/ tardiness : %d - %d = %d" %(job["end"], job.due, job["end"] - job.due))
