...
def SPT(names):
    ...
    return tardiness(spt), "SPT"

# -----------------------------EDD-----------------------------------------

def EDD(names):
    edd = sorted(names, key=lambda name: name.due)
    return tardiness(edd) ,"EDD"
...
def best(cases):
    return sorted([tardiness(case) for case in cases], key = lambda seq : seq[1])[0]
...
def start_sa_ans(ini_seq, nei):
    ...
    while lapse//by <= Time//by :
        ...
        for _ in range(iteration_max):
            final_tardiness = best(localsearch(list(sequence), v, nei))
            if final_tardiness[1] < total_tardiness:
                sequence, total_tardiness = final_tardiness
            else:
        ...
        if iteration_compare >= iteration_max / 2 and v < search_max:
            v += 1
        sa_ans_result=data_collection(sa_ans_result,lapse,total_tardiness)
        lapse = time.time() - ini_time

    return sequence, sa_ans_result
...
def start_ga(ini_seq):
    ...
    return sequence, ga_result

def meta_heuristic(*args):
    spt = sorted(args[0], key=lambda name: name.processing_time)
    if args[1] == 0:
        return start_sa_ans(tardiness(spt), args[2])
    elif args[1] == 1:
        return start_iga(tardiness(spt))
    elif args[1] == 2:
