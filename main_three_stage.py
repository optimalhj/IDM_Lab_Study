...
def lot_stream(process, deliver, boms, ini_set, params):
    ...
    # 트럭 위치 추적 (time-expanded flow) : 트럭은 공장 간 이동시간을 들여야만 다른 공장에 나타남 (순간이동 X)
    ...
    # 사전식(lexicographic) 단계별 풀이 : 앞 단계의 최적값을 고정하고 다음 순위를 최소화
    # 1순위 makespan > 2순위 생산 작업 수(과잉 생산 억제) > 3순위 재고 유지비용(너무 이른 생산 억제) > 4순위 트럭 투입 대수 > 5순위 트럭 총 이동시간
    solver = cp_model.CpSolver()
    solver.parameters.max_time_in_seconds = params.get("time_limit", 300)
    def solve_stage(name, objective):
        md.minimize(objective)
        status = solver.Solve(md)
        if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
            return status
        print(f"[{name}] {solver.value(objective)} ({solver.status_name(status)}, {solver.wall_time:.1f}s)")
        md.clear_hints()
        for var in [sem[fc][jt][k][i] for fc in sem for jt in sem[fc] for k in sem[fc][jt] for i in range(3)]:
            md.add_hint(var, solver.value(var)) # 이전 단계 해에서 출발
        return status

    # [1단계] makespan
    status = solve_stage("Stage 1 : Makespan", total_makespan)
    if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        print("X")
        return
    best_makespan = solver.value(total_makespan)
    md.add(total_makespan == best_makespan)

    # [2단계] 생산 작업 수
    total_job = sum(every_fc_num_job.values())
    status = solve_stage("Stage 2 : Total Job", total_job)
    if status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        print("X")
        return
    md.add(total_job == solver.value(total_job))

    # [3단계] 재고 유지비용 > 트럭 투입 대수 > 트럭 총 이동시간
    # Inventory Holding Cost : 제품 1개가 1시간 머무를 때마다 제품별 비용 발생, makespan 이전까지만 집계
    #   - 공장 재고(wip) : 도착했지만 아직 소비되지 않은 재료, lot 단위로 남은 재고, 먼저 완성되어 대기 중인 최종 제품
    #   - 운송 중 재고 : 트럭에 실려 이동 중인 제품 (출발 ~ 도착 사이)
    hold_cost = params["holding_cost"]
    holding = {}
    for fc in fc_wip_t.keys():
        for wip in fc_wip_t[fc].keys():
            holding[(fc, wip)] = hold_cost.get(wip, 0) * sum(fc_wip_t[fc][wip][t] for t in range(1, best_makespan))
    for fc in fc_deliver_to.keys():
        for fc_prime in fc_deliver_to[fc].keys():
            deliver_time = getattr(deliver, f"{fc}{fc_prime}")
            for jt in fc_deliver_to[fc][fc_prime]:
                holding[(f"{fc}→{fc_prime}", jt)] = hold_cost.get(jt, 0) * sum(delivering[fc][jt][t][fc_prime][lot_unit] * lot_unit * (min(t + deliver_time, best_makespan) - t) for t in range(1, best_makespan) for lot_unit in ini_set[fc][jt])
    total_holding = sum(holding.values())

    w_used = tps * horizon + 1 # 가중치 : 하위 항목의 최댓값 < 상위 항목 1단위
    w_hold = tps * w_used + tps * horizon + 1
    status = solve_stage("Stage 3 : Holding Cost & Truck", w_hold * total_holding + w_used * truck_used + truck_travel)
    ...
    if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        ...
        for (where, wip), expr in holding.items():
            if cost := solver.value(expr):
                print(f"    {where} {wip} : {cost} (= {hold_cost[wip]} x {cost // hold_cost[wip]} unit*time)")
        ...
        # 흐름(대수) 해를 트럭 한 대 한 대의 경로로 분해 : 출발지에 도착해 대기 중인 트럭만 출발 가능
...
def main():
    ...
    params = {
        ...
        "amount": 2,
        "horizon": 50,
        "time_limit": 300, # 2단계 풀이 제한 시간(초), 초과 시 그때까지의 최선해(FEASIBLE) 출력
